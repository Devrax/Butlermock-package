...
class StringPlaceholder {
    ...
    checkStringName(name) {
        ...
        if (name.toLowerCase().includes('image') || name.toLocaleLowerCase().includes('img'))
            return this.faker.image.url();
        if (name.toLowerCase().includes('email'))
            return this.faker.internet.email();
        return this.checkReservedString(name.toLocaleLowerCase());
    }
    checkReservedString(name) {
        const fullName = this.faker.person.fullName(), firstName = this.faker.person.firstName(), middleName = this.faker.person.middleName(), lastName = this.faker.person.lastName(), address = this.faker.location.streetAddress();
        return {
            name: fullName,
            full_name: fullName,
            fullName: fullName,
...
exports.default = StringPlaceholder;
